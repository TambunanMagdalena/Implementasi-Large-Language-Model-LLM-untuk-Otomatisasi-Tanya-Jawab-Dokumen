...
class PDFQAProcessor:
                ...
                if vector_store:
                    try:
                        ...
                        for doc, score in results:
                            if True: # VERY low threshold untuk testing
    ...
    def truncate_context(self, text, max_tokens=300):
    ...
    def generate_answer(self, context_docs, question):
        """Generate answer dengan context yang lebih ketat"""
        if not context_docs:
            return "Maaf, tidak menemukan informasi yang relevan dalam dokumen."

        # Hanya ambil 2 dokumen terbaik, bukan 3
        context_parts = []
        for doc in context_docs[:2]:  # KURANGI dari 3 ke 2
            content = doc.page_content
            # Clean content lebih agresif
            content = re.sub(r'\[.*?\]', '', content)  # Hapus semua metadata tags
            content = re.sub(r'\s+', ' ', content).strip()
            
            if content and len(content) > 20:
                # Truncate setiap dokumen lebih ketat
                truncated = self.truncate_context(content, max_tokens=150)
        ...
        context = "\n".join(context_parts)
        context = self.truncate_context(context, max_tokens=250)  # LEBIH KETAT

        # Prompt yang lebih sederhana
        prompt_template = """Jawab pertanyaan berdasarkan teks:
